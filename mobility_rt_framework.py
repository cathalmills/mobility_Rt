...
-----------
    import numpy as np
    from mobility_rt_framework import MobilityRtFramework, discretise_gamma
...
def compute_R0_from_beta(
    ...
    R0_mat = infectiousness_profile.sum() * populations[np.newaxis, :] * K_base
...
def estimate_beta_from_growth_rate(
    ...
    *,
    ...
    def _el(R0_val):
        return R0_val * float(np.sum(p * np.exp(-r_observed * days))) - 1.0
...
def compute_R_matrix(
    ...
    return infectiousness_profile.sum() * S[np.newaxis, :] * K_base
...
def spectral_analysis(R_mat: np.ndarray) -> dict:
    ...
    v = np.abs(evec_r[:, ir[0]]).real.astype(float)
    v /= (v.sum() + 1e-300)

    # Stable distribution v*: right eigvec of R_mat^T  (manuscript's v*)
    vs = np.abs(evec_l[:, il[0]]).real.astype(float)
    vs /= (vs.sum() + 1e-300)
...
def compute_generation_times(
    ...
    R_mat  = p.sum() * S[np.newaxis, :] * K_base
    ...
    for a in range(max_days):
        g_pw[a] = p[a]   # same for all (k,j)

    g_out  = np.tile(p[:, np.newaxis], (1, N))
    g_in   = np.tile(p[:, np.newaxis], (1, N))
    g_net  = p.copy()
...
def estimate_R_independent(
    ...
    pa, pb = 1.0, 5.0   # Gamma(1,5) prior → mean = 0.2
    for j in range(N):
...
def euler_lotka_r(rho: float, g_tilde: np.ndarray) -> float:
    """Solve Euler-Lotka 1 = R(t) Σ g(a) e^{-ra} for r."""
    if rho <= 0 or g_tilde.sum() < 1e-15:
        return np.nan
    days = np.arange(len(g_tilde))
    def f(r): return rho * float(np.sum(g_tilde * np.exp(-r * days))) - 1.0
    try:
        if abs(rho - 1.0) < 1e-8:
            return 0.0
...
def run_all_tests(verbose: bool = True) -> bool:
    """
    Run the full validation suite for mobility_rt_framework.
    ...
    for rho_test in [0.5, 1.0, 2.0]:
        ...
        if not np.isnan(r_sol):
            days = np.arange(len(p))
            resid = rho_test * float(np.sum(p * np.exp(-r_sol * days))) - 1.0
            t.approx(f"EL_identity_{rho_test}", resid, 0.0, atol=1e-6)

    # 12. Universal GT property: g^{kj} = p for all (k,j)
    gt = compute_generation_times(f, S, pops, p, lw, lb)
    for a in range(25):
        t.check(f"universal_gt_a{a}",
                np.allclose(gt["g_pairwise"][a], p[a], atol=1e-12),
